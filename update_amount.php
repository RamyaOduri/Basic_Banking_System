<?php
	...
	$db=mysqli_select_db($con,"c_details");
	...
	$sql = "SELECT Balance FROM customer_details WHERE name = '$sender'";
	...
	$sql = "SELECT Balance FROM customer_details WHERE name = '$receiver'";
	$result = mysqli_query($con,$sql);
	$row = mysqli_fetch_array($result);
	...
	if($sender_amount >= $amount){
		...
		$sql = "UPDATE customer_details set Balance = $amount_of_sender where name = '$sender'";
		$result = mysqli_query($con,$sql);

		$amount_of_receiver = $receiver_amount + $amount;
		$sql = "UPDATE customer_details set Balance = $amount_of_receiver where name = '$receiver'";
		$result = mysqli_query($con,$sql);
